...
```{r Librerias, results = 'hide',echo=FALSE}
...
```{r unir en tres tablas,echo=FALSE}
## por fuente 
...
Tomando la media de observaciones por a??o para el periodo de 2003 a 2013, vemos en la siguiente gr??fica que el mes de Marzo es el que tiene m??s casos en promedio. De hecho, parece que los meses m??s calidos (Mayo, Marzo, Junio, Julio, Agosto, Abril) son los ??nios que logran rebasar la media global, con la excepci??n de Octube y Enero. 

El mes con menor n??mero de casos en promedio es, por mucho, Diciembre. Esto salta a la vista pues n se espera que la diabetes sea una enfermedad estacional. Probablemente haya otra variable que explique este fen??meno, como en qu?? momento se busca el tratamiento en lugar de en qu?? momento se manifesta el trastorno.
...
Esta gr??fica se observa muy consistente con lo esperado. El grupo que en promedio registra m??s casos en el periodod es el de cinciuenta a cincuenta y nueve a??os. Seguido cercanamente de el de veinticino a cuarenta y cuatro y el de sesenta y cinco en adelante. Es inesperado que en el grupo de sesenta a sesenta y cuatro haya menos casos en promedio que entre los dos que lo rodean (cincuenta a cincuenta y nueve y sesenta y cinco en adelante). Esto se debe posiblemente a que los grupos de edad no comprenden los mismos per??odos de tiempo, dejando que ambos grupos aleda??os al de sesenta a sesenta y cuatro acumulen muchos m??s casos pues abarcan per??odos mucho m??s grandes. Como sugerencia para mejorar el an??lisis a futuro est?? el normalizar los grupos de edad para que comprendan todos per??odos del mismo tiempo; as?? eliminando el sesgo que esto introduce.

No se observan casos significativos hasta despu??s de los veinticinco a??os, pero no podemos saber con exactitud en qu?? edad comienzan la mauor??a de los casos por lo largo de ese grupo, que comprende casi veinte a??os.
...
##Media de 2003:2013 por fuente de notificaci??n

Sobresale significativamente la SSA como la mayor fuente de notificaci??n de casos, en promedio. Seguida de IMMS, ISSTE y OTRAS; SEDENA, PEMEX, SEMAR y DIF se encuentran muy por debajo de la media global, probablemente porque se trata se proveedores de servicios de salud que no atienden a la poblaci??n en general sino a grupos especializados. 

```{r,echo=FALSE}
ggplot(validacion_fuente,aes(x=fuente,y=media))+geom_bar(stat="identity")+theme(axis.text.x=element_text(angle=90,size=9))+scale_fill_manual(values=c("#b75b5b", "#7a3d3d", "#eac1c1","#d17575","#db9393","#663333","#3d1e1e","#000000","#663333","#e5b2b2","#a35151","#e0a3a3"))+geom_abline(intercept=as.numeric(mean(unlist(validacion_fuente$media))),size=2)+coord_flip()+ggtitle("Media de observaciones anuales entre 2003 y 2013, por fuente de notificaci??n")
```
