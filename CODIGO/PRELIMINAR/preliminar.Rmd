...
fuente <- within(fuente,{
                         ...
                         fuente <- gsub("Institución............................................................................................DIF","DIF", fuente)
...
mes <- within(mes,{
                         ...
                         Estado <- gsub("Baja California Sur ","Baja California Sur",Estado)
                         Estado <- gsub("San Luis Potosi","San Luis Potosí�",Estado)
                         Estado<-gsub("San Luis Potosí ","San Luis Potosí",Estado)
                         Estado<-gsub("México ","México",Estado)
                         Estado<-gsub("Michoacán ","Michoacán",Estado)
                         Estado<-gsub("Nuevo León ","Nuevo León",Estado)
                         Estado<-gsub("Yucatán ","Yucatán",Estado)
                         Estado<-gsub("Queretaro","Querétaro",Estado)
                         Estado<-gsub("San Luis Poto.*","San Luis Potosí",Estado)
...
## sumas de observaciones totales por año

transf <- c(2003:2013)
suma_obs_fem_an <- NULL
for (i in 1:11){
  suma_obs_fem_an[i] <- sum(subset(fuente, año ==transf[i] & genero == "femenino", select = "observaciones"), na.rm = TRUE)
...
for (i in 1:11){
  suma_obs_mas_an[i] <- sum(subset(fuente, año ==transf[i] & genero == "masculino", select = "observaciones"), na.rm = TRUE)
  
  }

suma_obs_gen_an <- NULL
for (i in 1:11){
  suma_obs_gen_an[i] <- sum(subset(fuente, año ==transf[i] & genero == "general", select = "observaciones"), na.rm = TRUE)
  }
...
columnas <- c("hombres","mujeres","suma hombres y mujeres","general","coinciden","media")

## data frames validaciones
validacion_fuente <- data.frame(suma_fuente_mas, suma_fuente_fem, suma_fuente_masfem, suma_fuente_gen, verd_fuente,media_fuente_gen, row.names = Fuente)
colnames(validacion_fuente) <- columnas

validacion_mes <- data.frame(suma_mes_mas, suma_mes_fem, suma_mes_masfem, suma_mes_gen, verd_mes, media_mes_gen,row.names = Mes)
colnames(validacion_mes) <- columnas
validacion_grupo <- data.frame(suma_grupo_mas, suma_grupo_fem, suma_grupo_masfem, suma_grupo_gen, verd_grupo, media_grupo_gen,row.names = Grupo)
...
#    verdadero <- NULL
#    for (i in 1:11){
#      suma <- sum(subset(data, año == transf[i] & genero == "masculino")$observaciones,na.rm=TRUE) + sum(subset(data, año == transf[[i]] & genero == "femenino")$observaciones,na.rm=TRUE)
#      verdadero[[i]] <- sum(subset(data, año == transf[i] & genero == "general")$observaciones,na.rm=TRUE) == suma 
#   
#    }
#    all(verdadero)
#} 
...
ggplot(grupo,aes(x=Estado,y=observaciones))+geom_boxplot()+facet_wrap(~año+genero)
names(grupo)
ggplot(grupo,aes(x=año,y=observaciones),nrow=2)+geom_line()+facet_wrap(~genero) +theme(axis.text.x=element_text(angle=90))

ggplot(fuente,aes(x=Estado,y=observaciones))+geom_boxplot()+facet_wrap(~año+genero)

ggplot(fuente,aes(x=fuente,y=observaciones))+geom_boxplot()+facet_wrap(~año+genero)
ggplot(mes,aes(x=Estado,y=observaciones))+geom_boxplot()+facet_wrap(~año+genero)

ggplot(mes,aes(x=mes,y=observaciones),mean(y))+geom_boxplot()+facet_wrap(~año+genero)
```
```{r practica}
ggplot(fuente,aes(x=año,y=observaciones))+geom_line(aes(colour=fuente))+facet_wrap(~fuente)+theme(axis.text.x=element_text(angle=90))+theme(legend.position="none",strip.text = element_text(face = "bold"),axis.text.x = element_text(size=15), axis.text.y = element_text(size=15),strip.background = element_blank(),axis.text = element_text(colour="grey",axis.ticks=element_line(colour="grey",axis.title = element_text(colour="grey", size=12), plot.title = element_text(size =20)))
